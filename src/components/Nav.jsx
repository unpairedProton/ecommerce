...
function Nav() {
  ...
  return (
    <div
      className="w-fit  lg:relative
     top-0"
    >
      <div className="fixed top-4 left-4 z-99 w-fit h-fit bg-slate-950 p-2 rounded-lg  lg:hidden">
        <Checkbox func={setIsSideBarVisible}></Checkbox>
      </div>
      <div
        className={`fixed top-0
    ${
      !isSideBarVisible ? "-translate-x-full" : "translate-x-0"
    } min-h-screen   z-10 lg:sticky lg:translate-x-0 md:w-44  duration-300`}
      >
        <nav className="pt-12 top-0 w-44 gap-2 h-full min-h-screen flex flex-col justify-start items-center lg:pt-2  bg-slate-950 text-zinc-200">
          <div className="w-[90%]">
            <img
          ...
          <Link
            to={"/create"}
            className="p-1.5 rounded-md border-cyan-600 border-1 text-cyan-600"
          >
            Add new Product
          </Link>
            ...
            {distinct_category.map((cat, i) => {
              return (
                <Link
                  ...
                  className={`flex justify-start hover:bg-slate-800 ${
                    cat == categoryFromURL ? "bg-slate-800" : ""
                  } rounded px-1.5 py-1 items-center gap-2`}
                  key={i}
                >
                  <div
